...
for scan in csv_list:
    ...
    if snum not in snum_list:
        ...
        press_combine = dd.concat([dd.read_csv(f).assign(SN=f.split("\\")[3].split("_")[0]).assign(
            Date=f.split("\\")[3].split("_")[1]) for f in press_list])
        print(press_combine)
        press_combine.to_csv('%s_press.csv' % (snum), single_file=1)

        temp_combine = dd.concat([dd.read_csv(f).assign(SN=f.split("\\")[3].split("_")[0]).assign(
            Date=f.split("\\")[3].split("_")[1]) for f in temp_list])
        print(temp_combine)
